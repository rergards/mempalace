...
Claude Code skills in `.claude/skills/` automate common workflows:

| Skill | Purpose |
|-------|---------|
| `/start` | Session startup — verify env, load backlog |
| `/status` | Project status — commits, priorities |
| `/verify` | Pre-commit gate — pytest, ruff check/format |
| `/palace-health` | Diagnose/repair palace issues |
| `/task-plan` | Plan backlog items with Codex review |
| `/task-hardening` | Post-impl hardening with triage |
| `/doc-refresh` | Weekly docs maintenance |
| `/ship` | Verify-fix-push loop |
| `/release` | PyPI release workflow |
| `/entropy-gc` | Codebase entropy cleanup |
| `/mine` | Project mining with validation |
| `/bench` | Embedding benchmarks |

Shared modules in `.claude/skills/_shared/`:
- `mode-classification.md` — 5-axis lite/standard/strict triage
- `task-state.md` — persist state across context compaction
- `commit-checkpoint.md` — safe commit procedure

## Hooks

Pre-commit hooks in `.claude/settings.json`:
- Run pytest, ruff check, ruff format before commits
- Validate BACKLOG.yaml if staged
- Warn about doc updates when core files change

Post-edit hooks:
- Log edits to `/tmp/claude-edits.log`
- Auto-validate BACKLOG.yaml
- Suggest tests when core modules edited
